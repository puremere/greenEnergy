...
namespace greenEnergy.Controllers
{
    public class appController : System.Web.Http.ApiController
    {
                ...
                if (someObject != null || (model.slug == "app/verify" || model.slug == "app/setCode"))
                {
                    ...
                    else if (model.slug == "app/submitMoney")
                    {
                        ...
                        if (finalModel.status == 200)
                        {
                            ...
                            await finalizePayment(finalModel.payment, usi, Int32.Parse(flowIDsrt), Int32.Parse(doctorFlowID));
                        }
                ...
                if (Item.isLinkToMain == 1)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                foreach (var flowrelarion in flowListQuery)
                                {
        ...
        private async Task<string> setUserDynamicFormData(getURLVM model, Guid userID)
        {
            ...
            using (Context dbcontext = new Context())
            {
                try
                {
                    ...
                    if (model.data.ContainsKey("relation"))
                    {
                        ...
                        else
                        {
                            ...
                            if (model.data.ContainsKey("status"))
                            {
                                ...
                                {
                                    actiontitle = "ارجاع شده به پزشک تخصصی";
                                }
                            }
                    ...
                    newOrderFlow.meta = await doSaveForm(lstform, flowID, dbcontext);
                    await dbcontext.SaveChangesAsync();
        ...
        private async Task<string> setNewProfileByDoctor(getURLVM model, Guid userID)
        {
            try
            {
                ...
                using (Context dbcontext = new Context())
                {
                    ...
                    if (userToAdd == null)
                    {
                        userToAdd = await dbcontext.users.SingleOrDefaultAsync(x => x.name == fullname && x.phone == phone);
                        if (userToAdd == null)
                        {
                            string status = "0";
                            Guid statusID = dbcontext.verifyStatuses.FirstOrDefault().verifyStatusID;
        ...
        private async Task<paymentVM> paymentProcess(getURLVM model, Guid userID, string flowID)
        {
            ...
            response.payment = discount != null ? "1" : ""; // ینی پرداخت با کد تخفیف
            response.status = 200;
            return response;
        }
        ...
        private async Task<int> getUserActionCount(string userID)
        {
            ...
            using (Context dbcontext = new Context())
            {
                ...
                var flowListQuery = firstFlow.childFlows.Where(x => (x.formID == 31 || x.formID == 25) && x.status != 3).ToList();
                //دریافت لیست فلو ها
                flowListCount = flowListQuery.Select(x => x.parentID).Distinct().ToList().Count();
            }
