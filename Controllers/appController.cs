...

        {
            ...
            try
            {
                ...
                using (Context dbcontext = new Context())
                {
                    ...
                    {
                        page = response.Contents.First().appMeta.Replace("sample", ""),
                        initData = initdata,
                    };
            ...
            saveLog(model, JsonConvert.SerializeObject(finalResult), "getAppURL");
            return finalResult;
        }
        ...
        private async Task<viewVM> getData([FromBody] getURLVM model)
        {
            ...
            using (Context dbcontext = new Context())
            {
                ...
                try
                {
                        ...
                        if (urlDatas.Count() > 0)
                        {
                            ...
                            foreach (var item in urlDatas.Where(x => x.isCycle == 1))
                            {
                                ...
                                if (model.data != null)
                                {
                                    ...
                                    {
                                        list = await getDataFromFlowGeneral(firstID, sUserID, item, flowID, thirdID, model);
                                    }
                                }
                                else
                                {
                                ...
                                if (model.slug == "app/clientActionList")
                                {
                                ...
                                else if (model.slug == "app/clientRecommendationList")
                                {
                                    if (model.data == null)
                                    {
                                        Dictionary<string, string> data = new Dictionary<string, string>();
                                    ...
                                    {
                                        model.data["next"] = "recommendationCycle#go*a.go*app/showDynamicForm";
                                    }
                                    else
                                    {
                                        model.data.Add("next", "recommendationCycle#go*a.go*app/showDynamicForm");

                                    }
                                }
                                //else if (model.slug == "app/clientActionList")
                                //{
                                //    if (model.data == null)
                                //    {
                                //        Dictionary<string, string> data = new Dictionary<string, string>();
                                ...
                                //    if (model.data.ContainsKey("next"))
                                //    {
                                //        model.data["next"] = "actionCycle#go*a.go*app/showDynamicForm";
                                //    }
                                //    else
                                //    {
                                //        model.data.Add("next", "actionCycle#go*a.go*app/showDynamicForm");
                                ...
                                else if (model.slug == "app/clientDashboard")
                                {
                                    if (model.data.ContainsKey("next"))
                                    {
                                        model.data["next"] = "doctorListCycleView#go*a.reload*app/clientDashboard;payeshReminder#var*formType*3_var*reload*app/payeshChart_go*a.go*app/dynamicFormAssess";
                                    }
                                    else
                                    {
                                        model.data.Add("next", "doctorListCycleView#go*a.reload*app/clientDashboard;payeshReminder#var*formType*3_var*reload*app/payeshChart_go*a.go*app/dynamicFormAssess");
                                ...
                                else if (model.slug == "app/homePageDoctor")
                                {
                                    ...
                                    if (model.data.ContainsKey("next"))
                                    {
                                        model.data["next"] = "payeshHomeCycleForDoctor#putVar*to*app/homePageDoctor_go*a.go*app/doctorPatientInfo";

                                    }
                                    else
                                    {
                                        model.data.Add("next", "payeshHomeCycleForDoctor#putVar*to*app/homePageDoctor_go*a.go*app/doctorPatientInfo");
                                ...
                                else if (model.slug == "app/donePageDoctor")
                                {
                                    ...
                                    if (model.data.ContainsKey("next"))
                                    {
                                        model.data["next"] = "payeshCycleForDoctor#putVar*to*app/donePageDoctor_go*a.go*app/doctorPatientInfo";

                                    }
                                    else
                                    {
                                        model.data.Add("next", "payeshCycleForDoctor#putVar*to*app/donePageDoctor_go*a.go*app/doctorPatientInfo");
                                    }
                                }
                    ...
                    // productRetaledSections
                    if (model.slug.Contains("app/getProductFilter"))
                    {
                        if (model.data.ContainsKey("selectedForm"))
                        {
                            int selectedForm =Int32.Parse(model.data["selectedForm"]);

                            allDataDynamic.Add("stackProductFilter_visibilitysrt", "1");


                            List<formItemList> showForm = await getFormDataFromForm(selectedForm);
                    ...
                    else if (model.slug == "app/doctorPatientInfo")
                    {
                        ...
                        flowRelation relation2 = await dbcontext.flowRelations.Include(x => x.parentFlow).Include(x => x.parentFlow.NewOrderFields).OrderByDescending(x => x.flowRelationID).FirstOrDefaultAsync(x => x.childID == payeshFLow.newOrderFlowID && x.formID == 31);
                        ...
                        if (relation2 != null)
                        {
                            if (relation2.parentFlow.NewOrderFields != null)
                            {
                                var descc = relation2.parentFlow.NewOrderFields.SingleOrDefault(x => x.name == "description");
                    ...
                    else if (model.slug == "app/lineChart")
                    {
                        string fromWho = model.data["from"];
                        List<chartList> chartList = new List<chartList>();
                        if (fromWho == "7")
                        {
                            Guid doctorID = new Guid(model.firstID);
                            form doctorPayeshForm = await dbcontext.forms.SingleOrDefaultAsync(x => x.userID == doctorID && x.formTypeID == 3);
                            Guid clientUserID = new Guid(model.userID);
                            List<int> userPayeshFlows = await dbcontext.newOrderFlows.Where(x => x.userID == clientUserID && x.formID == doctorPayeshForm.formID).Select(x => x.newOrderFlowID).ToListAsync();
                            ...
                            chartList = await getLineChartData(model, formItem, userPayeshFlows);
                        }
                        else if (fromWho == "5")
                        {
                            int doctorSelectedFlow = Int32.Parse(model.data["flowID"]);
                            newOrderFlow doctorSectedFlow = await dbcontext.newOrderFlows.SingleOrDefaultAsync(x => x.newOrderFlowID == doctorSelectedFlow);
                            var doctorSpecficPayeshForm = await dbcontext.forms.SingleOrDefaultAsync(x => x.formTypeID == 3 && x.userID == doctorSectedFlow.userID);

                            Guid clientUserID = new Guid(model.userID);
                            List<int> userPayeshFlows = await dbcontext.newOrderFlows.Where(x => x.userID == clientUserID && x.formID == doctorSpecficPayeshForm.formID).Select(x => x.newOrderFlowID).ToListAsync();
                            List<formItemVM> formItem = await dbcontext.formItems.Where(x => x.formID == doctorSpecficPayeshForm.formID).Select(x => new formItemVM { formItemID = x.formItemID, itemName = x.itemName }).ToListAsync();
                            chartList = await getLineChartData(model, formItem, userPayeshFlows);

                        }
                        else if (fromWho == "1")// ینی دکتر میانی
                        {
                            int userPayeshFlow = Int32.Parse(model.data["flowID"]);
                            newOrderFlow userSelectedPayeshFlow = await dbcontext.newOrderFlows.SingleOrDefaultAsync(x => x.newOrderFlowID == userPayeshFlow);


                            Guid clientUserID = new Guid(model.userID);
                            List<int> userPayeshFlows = await dbcontext.newOrderFlows.Where(x => x.userID == clientUserID && x.formID == userSelectedPayeshFlow.formID).Select(x => x.newOrderFlowID).ToListAsync();
                            List<formItemVM> formItem = await dbcontext.formItems.Where(x => x.formID == userSelectedPayeshFlow.formID).Select(x => new formItemVM { formItemID = x.formItemID, itemName = x.itemName }).ToListAsync();
                            chartList = await getLineChartData(model, formItem, userPayeshFlows);
                    ...
                    else if (model.slug == "app/clientDashboard")
                    {
                        ...
                        allDataDynamic.Add("currentDateLabel_textsrt", dateTimeConvert.ToPersianDateNoHourString(DateTime.Now));
                        ...
                        if (model.data.ContainsKey("flowID"))
                        {
                            //object userOBJID;
                            //Request.Properties.TryGetValue("UserToken", out userOBJID);
                            Guid userID = new Guid(model.userID.ToString());
                            int FlowID = Int32.Parse(model.data["flowID"]);
                    ...
                    else if (model.slug == "app/clientDoctorSpecificButtons")
                    {
                        if (!model.data.ContainsKey("fromDoctor"))
                        {
                            allDataDynamic.Add("spCommentList_visibilitysrt", "0");

                        }
                        if (model.data.ContainsKey("fromDoctor"))
                        {
                            allDataDynamic.Add("payeshListStack_visibilitysrt", "0");
                    ...
                    else if (model.slug == "app/picList")
                    {
                        if (model.data.ContainsKey("fromDoctor"))
                        {
                            allDataDynamic.Add("addAssessButton_1_visibilitysrt", "0");

                        }
                    }
                    else if (model.slug == "app/testList")
                    {
                        if (model.data.ContainsKey("fromDoctor"))
                        {
                            allDataDynamic.Add("addAssessButton_1_1_visibilitysrt", "0");
                        }

                    }
                    else if (model.slug == "app/assessList")
                    {
                        if (model.data.ContainsKey("fromDoctor"))
                        {
                            allDataDynamic.Add("addAssessButton_visibilitysrt", "0");
                        }
                    ...
                    else if (model.slug == "app/setFormByClient")
                    {
                        ...
                        showFormInputVM fmodel = new showFormInputVM();
                        fmodel.flowID = Int32.Parse(flowId);
                        List<formItemList> showForm = await getFormItems(fmodel);
                        showFormAllVM formModel = new showFormAllVM()
                        {
                            name = "flowForm",
                    ...
                    else if (model.slug == "app/getOrderListManager")
                    {
                        ...
                        //setFlow_orderListManager_cycleActionsrt = "[{\"type\" : \"a.putVar\", \"varName\":\"orderID\",\"value\":\"" + x.newOrderID + "\"},{\"type\":\"a.go\",\"to\":\"app/requestFormSetNewFLow\"}]"
                        //historyButton_orderListManager_cycleActionsrt = "[{\"type\" : \"a.putVar\", \"varName\":\"orderID\",\"value\":\"" + x.newOrderID + "\"},{\"type\":\"a.go\",\"to\":\"app/getOrderFlowManager\"}]" 
                        List<cycleStackView_orderListManagerVM> cyclelist = await dbcontext.NewOrders.Include(x => x.newOrderStatus).Select(x => new cycleStackView_orderListManagerVM {
                            orderID_valuesrt = x.newOrderID+"",
                            setFlow_orderListManager_visibilitysrt = x.newOrderStatus.statusCode == "1" ? "1" : "0",
                            orderStatus_valueTextsrt = x.newOrderStatus.title, 
                            orderStatus_valueColorsrt = x.newOrderStatus.statusCode == "1" ? "cRed" : "cGreen", 
                            projectName_orderListManager_valueTextsrt = x.orderName,
                            orderDate_orderListManager_valueTextsrtdate = x.creationDate}
                        ).ToListAsync();
                        parentlist = new List<parent>();
...
//rejectOrderButton_cycleActionsrt = x.isAccepted == "0" ? "[{\"type\" : \"a.putVar\", \"varName\":\"flowID\",\"value\":\"" + x.newOrderFlowID + "\"},{\"type\":\"a.go\",\"isPopUp\":1,\"width\":300,\"height\":220,\"to\":\"app/clientRejectFlow\"}]" : "[]",                            
                            Select(x => new clientorderListPending
                            {
                                ...
                                orderDate_valueTextsrt = x.actionDate+"",
                                ...
                                productRegistration_visibilitysrt = x.isAccepted == "1" ? "1" : "0",
                                ...
                                costRegistration_visibilitysrt =  x.isAccepted ==  "1" ? "1" : "0",
        ...
        public async Task<appResponse> setFormData([FromBody] getURLVM model)
        {
            ...
            try
            {
                ...
                if (someObject != null || (model.slug == "app/verify" || model.slug == "app/setCode"))
                {
                    if (model.slug == "app/verify")
                    {
                        ...
                        if (model.data.ContainsKey("sendCodeAgain"))
                        {
                            actionResonse action1 = new actionResonse()
                            {
                                type = "a.message",
                    ...
                    {
                        Guid usi = new Guid(model.firstID);
                        ...
                        if (model.data.ContainsKey("level2"))
                        {
                            ...
                            if (nextList[0] == "reload")
                            {
                                ...
                                string reloadTo = nextList[1];
                                // اگر to مشخص شده باشد از ان استفاده میکنیم
                                if (model.data.ContainsKey("to"))
                                {
                                    reloadTo = model.data["to"];
                                }
                                actionResonse reload = new actionResonse()
                                {
                                    type = "a.reload",
                    ...
                    else if (model.slug == "app/changeRelationStatus")
                    {
                        Guid usi = new Guid(model.userID);
                        string rsp = await changeRelationStatus(model, usi);
                    ...
                    // product sepecific section 
                    else if (model.slug == "app/returnProductCode")
                    {
                        Guid usi = new Guid(model.userID);
                        string phoneSended = "";
                        using (Context dbcontext = new Context())
                    ...
                    else if (model.slug == "app/setCatVariable")
                    {
                        ...
                        List<actionResonse> actionlst = new List<actionResonse>();
                        string id = selectedForm.Trim(',').Split(':').ToList().First();
                        using (Context dbccontext = new Context())
                        {
                            Guid idguid = new Guid(id);
                            orderOption selectedOrderOption = await dbccontext.orderOptions.SingleOrDefaultAsync(x => x.orderOptionID == idguid);
                            string formID = selectedOrderOption.Value;
                            actionResonse goaction = new actionResonse()
                            {
                                type = "a.putVar",
                                varName = "selectedForm",
                                value = formID,
                            };
                            actionlst.Add(goaction);
                            goaction = new actionResonse()
                    ...
                    else if (model.slug == "app/setNewProduct")
                    {
                        using (Context dbcontext = new Context())
                        {
                            Guid usi = new Guid(model.firstID);
                            string rsp = await setProduct(model, usi, dbcontext);
                            List<actionResonse> actionlst = new List<actionResonse>();
                    ...
                    else if (model.slug == "app/submitMoney")
                    {
                        ...
                        if (finalModel.status == 200)
                        {
                            ...
                            //}
                            string doctorFlowID = model.data["flowID"];
                            await finalizePayment(finalModel, usi, Int32.Parse(flowIDsrt), Int32.Parse(doctorFlowID));
                        }
                        ...
                        if (finalModel.payment == "2")
                        {
                            ...
                            {
                                type = "a.browser",
                                to = "https://mypals.ir/connect/ReqestForPaymentMellat/" + finalModel.paymentRecortID,
                                value = "https://mypals.ir/connect/ReqestForPaymentMellat/" + finalModel.paymentRecortID,
                            };
                            newgoaction.actions.Add(reload);
                            actionlst.Add(newgoaction);
                        }
            ...
            {
                actionResonse actoin = new actionResonse();
                actoin.type = "error";
                string ER = e.InnerException != null ? e.InnerException.Message : e.Message;
                actoin.value = ER;
                List<actionResonse> lst = new List<actionResonse>();
                lst.Add(actoin);
                result.actions = lst;
            ...
            saveLog(model, JsonConvert.SerializeObject(response), "setFormData");
            return response;
        }
        ...
        private async Task<flowDetailAll> getDataFromFlowDetaialGeneral(string userID, int flowID, int aboutUserFlow, Context dbcontext)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        newOrderFlow flw = await dbcontext.newOrderFlows.SingleOrDefaultAsync(x => x.newOrderFlowID == flowID);
                        if (!string.IsNullOrEmpty(flw.meta))
                        {
                ...
                if (meta.Count > 0)
                {
                    foreach (var metaDetail in meta)
                    {
                        ...
                        {
                            dic[metaDetail.Key] = metaDetail.Value;
        ...
        private async Task<List<flowDetailAll>> getDataFromFlowGeneral(string firstID, string userID, urlData Item, int? flowID, string thirdID, getURLVM model)
        {
            ...
            using (Context dbcontext = new Context())
            {
                if (Item.isLinkToMain == 1)
                {
                    ...
                    if (Item.formID == null)// اینجا ینی فرم خاصی نیست و داینامیک است پس باید ای دی داینامیک دربیاد
                    {
                        ...
                        // اینجا فک کنم میخواد بگه ما شاید اطلاعات مرتبط با خود طرفو بگریم 
                        ...
                        if (selectedFlow.formID == 5)// ینی خود فرم نیست که کلیک شده لیست کاربری دکتر کلیک شده پس
                        {
                            Guid doctorGU = new Guid(firstID);
                            // اینجا ینی فلو کاربر درگیر شده نه فرمی که ثبت کرده
                            dynamicForm = await dbcontext.forms.SingleOrDefaultAsync(x => x.formTypeID == Item.formTypeID && x.userID == doctorGU);
                    ...
                    {
                        // انیجا در نظر گرفته فلو آی دی مال دکتر بوده
                        newOrderFlow doctorFLow = await dbcontext.newOrderFlows.SingleOrDefaultAsync(x => x.newOrderFlowID == flowID);
                        string fromWho = "";
                        if (model.data.ContainsKey("from"))
                        {
                            fromWho = model.data["from"];
                        }

                        if (fromWho == "7")
                        {
                            Guid doctroGUID = new Guid(firstID);

                            doctorFLow = await dbcontext.newOrderFlows.SingleOrDefaultAsync(x => x.formType == 7 && x.userID == doctroGUID);
                        }
                        else if (fromWho == "5")
                        {
                            // همین الان دکتر فلو انتخاب شده
                        }
                        else if (fromWho == "0")
                        {
                            // باز هم فلو انتخاب شده برای همین 
                        }

                        // این به هر ترتیب کاربره
                    ...
                    else
                    {
                        if (Item.isCustom == 1)// کالکشن شرایط نال نیست
                        {
                            ...
                            else if (Item.name == "userListCycle")
                            {
                                ...
                                List<int> userFLowList = await dbcontext.flowRelations.Where(x => x.status == 1 && x.formID == 7 && x.parentID == mainDoctorFlow.newOrderFlowID).Select(x => x.childID).Distinct().ToListAsync();
                        ...
                        else
                        {
                            ...
                            List<int?> idlisNull = new List<int?>();
                            if (!string.IsNullOrEmpty(Item.formIDString))
                            {
                                idlist = Item.formIDString.Split(',').Select(int.Parse).ToList();
                                idlisNull = new List<int?>(idlist.Count);
                            ...
                            var flowListQuery = !string.IsNullOrEmpty(Item.formIDString) ? firstFlow.childFlows.Where(x => idlisNull.Contains(x.formID)) : firstFlow.childFlows.Where(x => x.formID == Item.formID);
                            // در این بخش سعی میکنیم فلوهایی را بیاوریم که فقط مرتبط با پزشک یا نفر خاص است
                            // ینی کریتور درآن لحاظ میشود


                            if (model.data.ContainsKey("justCreator"))
                            {
                                int flowIDCreator = 0;
                                string fromWho = "";
                                ...
                                {
                                    Guid doctroGUID = new Guid(firstID);
                                    var doctorFLow = await dbcontext.newOrderFlows.SingleOrDefaultAsync(x => x.formID == 7 && x.userID == doctroGUID);
                                    flowIDCreator = doctorFLow.newOrderFlowID;
                                }
                                else if (fromWho == "5")
                                {
                                    flowIDCreator = Int32.Parse(model.data["flowID"]);
                                    // همین الان دکتر فلو انتخاب شده
                                }
                                else if (fromWho == "1") // ینی میانی
                                {
                                    int formInsertedFlow = Int32.Parse(model.data["flowID"]);
                                    newOrderFlow clientPayeshFlow = await dbcontext.newOrderFlows.SingleOrDefaultAsync(x => x.newOrderFlowID == formInsertedFlow);

                                    var doctorPayeshForm = await dbcontext.forms.SingleOrDefaultAsync(x => x.formID == clientPayeshFlow.formID);
                                    var doctorFLow = await dbcontext.newOrderFlows.SingleOrDefaultAsync(x => x.formID == 7 && x.userID == doctorPayeshForm.userID);
                                    flowIDCreator = doctorFLow.newOrderFlowID;

                                }


                                // اینجا حالت کاربره چون میخواد لیست ببینه فلو آی دی دکتر میاد باش

                                flowListQuery = flowListQuery.Where(x => x.actorFlowID == flowIDCreator).ToList();
                            }


                            if (!string.IsNullOrEmpty(Item.conditionRelationStatus))
                            {
                                List<int> relationStatuslist = Item.relationStatus.Split(',').Select(int.Parse).ToList();
                                List<int?> relationArrayNullable = relationStatuslist.Cast<int?>().ToList();
                                foreach (var item in relationStatuslist)
                                {
                                    relationArrayNullable.Add(item);
                                }
                                if (Item.conditionRelationStatus == "0")
                                {
                                    flowListQuery = flowListQuery.Where(x => !relationArrayNullable.Contains(x.status));
                                }
                                else
                                {
                                    flowListQuery = flowListQuery.Where(x => relationArrayNullable.Contains(x.status));
                            ...
                            {
                                flowListQuery = flowListQuery.Where(x => x.parentFlow.creationDate >= DateTime.Today);
                                List<int> lst = flowListQuery.Select(x => x.parentID).Distinct().ToList();
                            }
                            if (Item.isOrderByDate == 1)
                            {
                                flowListQuery = flowListQuery.OrderByDescending(x => x.childFlow.changeStatusDate);
                                List<int> lst = flowListQuery.Select(x => x.parentID).Distinct().ToList();

                            }
                            else if (Item.isOrderByDate == 2) // ینی بر اساس فقط فلو آی دی دیسندینگ
                            {
                                flowListQuery = flowListQuery.OrderByDescending(x => x.childFlow);
                                List<int> lst = flowListQuery.Select(x => x.parentID).Distinct().ToList();
                            }
                            else if (Item.isOrderByDate == 3)// ینی بر اساس فقط فلو آی دی اسندینگ
                            {
                                flowListQuery = flowListQuery.OrderBy(x => x.childFlow);
                                List<int> lst = flowListQuery.Select(x => x.parentID).Distinct().ToList();
                            }
                            else
                            {
                            ...
                            // یک تابع برای اینکه ریلیشن رو استتوسشو عوض کنی

                            if (Item.name == "actionCycle" || Item.name == "recommendationCycle")
                            {
                                var llst = flowListQuery.ToList();
                                foreach (var flowrelarion in llst)
                                {
                                    flowrelarion.status = 3;
                                }
                                await dbcontext.SaveChangesAsync();
                                //دریافت لیست فلو ها
                                List<flowRelation> flowlist = flowListQuery.ToList();
                                List<int> flowListID0 = flowListQuery.Select(x => x.parentID).ToList();
                                response = await getDataFlowGenral(flowListID0, Item);
                                return response;
                            }
                ...
                else
                {
                    ...
                    if (Item.formTypeID != null)
                    {
                        ...
                        {
                            lstt = await lsttquery.Select(x => x.newOrderFlowID).OrderByDescending(x => x).ToListAsync();
                        }
                        else if (Item.isOrderByDate == 3)// ینی بر اساس فقط فلو آی دی اسندینگ
                        {
                            lstt = await lsttquery.Select(x => x.newOrderFlowID).OrderBy(x => x).ToListAsync();
                        }
                        else
                        {
        ...
        private async Task<List<flowDetailAll>> getDataFlowGenral(List<int> flows, urlData Item)
        {
            ...
            if (flows != null)
            {
                using (Context dbcontext = new Context())
                {
                    ...
                    foreach (var flowID in flows) //.OrderByDescending(x=>x)
                    {
                        ...
                        {
                            if (!string.IsNullOrEmpty(selectedFLow.meta))
                            {
                                Dictionary<string, string> flowValues = JsonConvert.DeserializeObject<Dictionary<string, string>>(selectedFLow.meta);
                                foreach (var field in Item.formFields.Trim().Split(',').ToList())
                                {
                                    if (!string.IsNullOrEmpty(field))
                                    {
                                        ...
                                        if (flowValues.ContainsKey(firstPart))
                                        {
                                            ...
                                            {
                                                dic.Add(field, flowValues[firstPart].Split(':').ToList()[1]);
                        ...
                        {
                            List<string> relationString = Item.extraRelation.Split(';').ToList();

                            try
                            {
                                foreach (var relation in relationString)
                                {
                                    List<string> lll = relation.Trim(',').Split(',').ToList();
        ...
        private async Task<List<int>> getSearchedFlow(getURLVM model, Guid userID)
        {
            ...
            using (Context dbcontext = new Context())
            {
                ...
                if (lstform.Where(x => x.key != "relationStatus").Count() > 0)
                {
        ...
        private async Task<List<int>> findFlowsForSearch(getURLVM model, Context dbcontext, Guid userID)
        {
            ...
            List<int> response = new List<int>();
            int StatusCode = -1;
            if (lstform.Any(x => x.key.Contains("relationStatus")))
            {
                string relationstatusstring = lstform.First(x => x.key == "relationStatus").value;
                if (relationstatusstring.Contains("فعال"))
                {
                    StatusCode = 1;
                }
                if (relationstatusstring.Contains("غیر فعال"))
                {
                    StatusCode = 0;
                }
            ...
            if (model.data.ContainsKey("name"))
            {
                ...
                else if (name == "searchUserDoctor")
                {
                    ...
                    {
                        // پشتیبان
                        var responseqeuery = dbcontext.flowRelations.Where(x => x.formID == 7).AsQueryable(); // اینجا ینی همه 
                        if (StatusCode != -1)
                        {
                            responseqeuery = responseqeuery.Where(x => x.status == StatusCode);

                        }
                        if (responseqeuery != null)
                        {
                            //var list = responseqeuery.ToList();
        ...
        private async Task<string> setUserDynamicFormData(getURLVM model, Guid userID)
        {
            ...
            using (Context dbcontext = new Context())
            {
                try
                {
                    ...
                    if (model.data.ContainsKey("withoutFlow"))
                    {
                        // دراین حالت حتما userToken داره میاد
                        // ینی دکتر داره میبینه 
                        Guid userSelectedGUID = new Guid(model.userID);
                        Guid doctorGUID = new Guid(model.firstID);
                        user UserSelectedByDoctor = await dbcontext.users.SingleOrDefaultAsync(x => x.userID == userSelectedGUID);
                        user doctor = await dbcontext.users.SingleOrDefaultAsync(x => x.userID == doctorGUID);
                        int userBaseFormID = dynamicMethods.getBaseFlowID(UserSelectedByDoctor.userType);
                        int doctorBaseFormID = dynamicMethods.getBaseFlowID(doctor.userType);
                        newOrderFlow UserSelectedFlow = await dbcontext.newOrderFlows.SingleOrDefaultAsync(x => x.userID == userSelectedGUID && x.formID == userBaseFormID);//اولین فلو ثبت اطلاعات کاربر
                        newOrderFlow doctorFlow = await dbcontext.newOrderFlows.SingleOrDefaultAsync(x => x.userID == doctorGUID && x.formID == doctorBaseFormID);//اولین فلو ثبت اطلاعات کاربر

                        DateTime creationTIME = DateTime.Now;
                        // انیجا فلو به نام دکتر ثبت میشه 
                        newOrderFlow = new newOrderFlow()
                        {
                            creationDate = creationTIME,
                            actionDate = DateTime.Now,
                            isFinished = "1",
                            serventPhone = doctor.phone,
                            userID = doctorGUID,
                        ...
                        // اولین ریلیشن به نام دکتر
                        flowRelation nr = new flowRelation()
                        {
                            childID = doctorFlow.newOrderFlowID,
                            parentID = newOrderFlow.newOrderFlowID,
                            actorFlowID = doctorFlow.newOrderFlowID,
                            status = 1,
                        ...
                        {
                            childID = UserSelectedFlow.newOrderFlowID,
                            parentID = newOrderFlow.newOrderFlowID,
                            actorFlowID = doctorFlow.newOrderFlowID,
                            status = 1,
                            childEndDate = DateTime.Now,
                        ...
                        await dbcontext.SaveChangesAsync();
                        newOrderFlow.meta = await doSaveForm(lstform, newOrderFlow.newOrderFlowID, dbcontext);
                        await dbcontext.SaveChangesAsync();
                    }
                    else
                    {
                        ...
                        Guid firstIdGUID = new Guid(model.firstID);
                        user firstUser = await dbcontext.users.SingleOrDefaultAsync(x => x.userID == firstIdGUID);
                        ...
                        if (model.data.ContainsKey("relation"))
                        {
                            ...
                            if (formID == 31)
                            {
                                ...
                                if (optionRow != null)
                                {
                                    var baseList = optionRow.value.Trim(',').Split(',').ToList();
                                    string final = "";
                                    foreach (var bitem in baseList)
                                    {
                                        var liststring = bitem.Trim(':').Split(':').ToList();
                                        final += liststring[1] + ",";
                                    }

                                    actiontitle = final.Trim(',');
                                }
                                flowLog newflowlog = new flowLog()
                                {
                                    ...
                                    actorFlowID = userFlow.newOrderFlowID,
                                    actorUserType = firstUser.userType,
                                    formID = formID,
                            ...
                            else
                            {
                                ...
                                // انیجا میخوایم از استتوس ارسال شده کمک بگیریم و یه لاگ ایجاد کنیم
                                // اینجا ما برای همه فرم ها میخوایم لاگ بینویسیم

                                if (model.data.ContainsKey("status"))
                                {
                                    string laststatus = model.data["status"];
                                    if (laststatus == "6")
                                    {
                                        actiontitle = "ارجاع اورژانسی به پزشک متخصص";
                                    }
                                    else if (laststatus == "3")
                                    {
                                        actiontitle = "ارجاع شده به پزشک متخصص";
                                    }
                                }
                                ...
                                if (formID == 31)
                                {
                                    int userPayeshFlowID = Int32.Parse(model.data["relation"]);
                                    newOrderFlow userPayesh = await dbcontext.newOrderFlows.SingleOrDefaultAsync(x => x.newOrderFlowID == userPayeshFlowID);
                                    await sendNotif("", userPayesh.userID.ToString(), "اقدام فوری برای شما ثبت شده است", "reload", "app/clientActionList");
                                }
                            }
        ...
        private async Task<string> saveLog(getURLVM model, string response, string where)
        {
            string Content = "";
            using (Context dbcontext = new Context())
            {
                datalog model0 = new datalog()
                {
                    form = model.form,
                    data = JsonConvert.SerializeObject(model.data),
                    slug = model.slug,
                    response = response,
                    userID = model.firstID,
                    where = where
                };
                dbcontext.datalogs.Add(model0);
                await dbcontext.SaveChangesAsync();
            }
            return Content;
        }
        ...
        private async Task<string> setUserTipoxProfile(getURLVM model, Guid userID, Context dbcontext)
        {
            try
            {
                ...
                {
                    userInfoFlow.meta = await doSaveForm(lstform, userInfoFlow.newOrderFlowID, dbcontext);
                    await dbcontext.SaveChangesAsync();
                    return "";
                }
        ...
        private async Task<string> setUserProfile(getURLVM model, Guid userID, Context dbcontext)
        {
            try
            {
                ...
                if (userInfoFlow != null)
                {
                    ...
                    string meta = await doEditForm(lstform, userInfoFlow, dbcontext);
                    userInfoFlow.meta = meta;
                    await dbcontext.SaveChangesAsync();
                    return "";
                }
            ...
            try
            {
                ...
                using (Context dbcontext = new Context())
                {
                    ...
                    if (userToAdd == null)
                    {
                        ...
                        childFIleFLow.meta = await doSaveForm(lstform, childFIleFLow.newOrderFlowID, dbcontext);
                        await dbcontext.SaveChangesAsync();
            ...
            try
            {
                ...
                using (Context dbcontext = new Context())
                {
                    ...
                    if (userToAdd == null)
                    {
                        ...
                        newOrderFlow.meta = await doSaveForm(user5InfoDetail, newOrderFlow.newOrderFlowID, dbcontext); // اینجا اطلاعات کاربر با ججزئیات ثبت می شود
                        await dbcontext.SaveChangesAsync();
        ...
        private async Task<string> checkDoctorRegistered(string ID, int doctorFlow)
        {
            ...
            using (Context dbcontext = new Context())
            {
                ...
                flowRelation flowRelation = await dbcontext.flowRelations.SingleOrDefaultAsync(x => x.childID == userflow.newOrderFlowID && x.parentID == doctorFlow && x.formID == 7 && x.status == 1);
                ...
                using (Context dbcontext = new Context())
                {
                    ...
                    if (sendingUser.userType == "2")
                    {
                        ...
                        if (docflow != null)
                        {
                            ...
                            docflow.meta = await doEditForm(lstform, docflow, dbcontext);
                            await dbcontext.SaveChangesAsync();
                        }
                    ...
                    if (newuser == null)
                    {
                        ...
                        formItem newFormItem0 = new formItem()
                        {
                            formID = payeshForm.formID,
                            formItemTypeID = new Guid("75dbb88b-db55-4f4f-a4db-707b4557084a"), // متنی
                            isHidden = 0,
                            itemDesc = "توضیحات تکمیلی",
                            itemPlaceholder = "توضیحات تکمیلی خود را وارد کنید",
                            itemName = "description",
                            pageNumber = 0,
                            priority = 1,
                        };
                        dbcontext.formItems.Add(newFormItem0);
                        ...
                        if (fbs != null)
                        {
                            if (fbs.value == "1")
                            {
                                ...
                                Guid optionTime = new Guid("c2759b03-7b2c-417d-8b2e-02cc9a249f74"); //آیتم زمان اندازه گیری قند خون

                                formItem newFormItem2 = new formItem()
                                {
                                    formID = payeshForm.formID,
                                    ...
                                    isHidden = 0,
                                    isRequired = "on",
                                    itemDesc = "زمان اندازه گیری قند خود",
                                    itemPlaceholder = "زمان اندازه گیری قند خون را مشخص نمایید",
                                    itemName = "sugarTime",
                                    maxNumber = 0,
                                    minNumber = 0,
                                    pageNumber = 0,
                                ...
                                dbcontext.formItems.Add(newFormItem2);
                            }
                        ...
                        if (Temp != null)
                        {
                            if (Temp.value == "1")
                            {
                                ...
                                dbcontext.formItems.Add(newFormItem);
                                Guid optionTime = new Guid("a9962320-d1ba-493d-9e6e-e0110c43a91d"); //محل اندازه گیری دمای بدن
                                ...
                                formItem newFormItem2 = new formItem()
                                {
                                    ...
                                    isHidden = 0,
                                    isRequired = "on",
                                    itemDesc = "محل اندازه گیری دمای بدن",
                                    itemPlaceholder = "محل اندازه گیری دمای بدن مشخص نمایید",
                                    itemName = "tempPlace",
                                    maxNumber = 0,
                                    minNumber = 0,
                                    pageNumber = 0,
        ...
        private async Task<string> setNewProfileByDoctor(getURLVM model, Guid userID)
        {
            try
            {
                ...
                string fullname = lstform.SingleOrDefault(x => x.key == "fullname").value;
                string phone = methods.PersianToEnglish(lstform.SingleOrDefault(x => x.key == "phone").value);
                string IDCode = methods.PersianToEnglish(lstform.SingleOrDefault(x => x.key == "ID").value);
                string image = lstform.Any(x => x.key == "image") ? lstform.SingleOrDefault(x => x.key == "image").value : "";
                ...
                using (Context dbcontext = new Context())
                {
                    Guid docuid = new Guid(model.firstID);
                    newOrderFlow docflow = await dbcontext.newOrderFlows.SingleOrDefaultAsync(x => x.userID == docuid && x.formID == 7);
                    Dictionary<string, string> flowValues0 = JsonConvert.DeserializeObject<Dictionary<string, string>>(docflow.meta);
                    doctorfullname = flowValues0["fullname"];
                    user userToAdd = await dbcontext.users.SingleOrDefaultAsync(x => x.phone == phone || x.codeMelli == IDCode);
                    Guid partnerID = Guid.NewGuid();
                    if (userToAdd == null)
                    {
                        string status = "0";
                        ...
                        dbcontext.users.Add(newuser);
                        Dictionary<string, string> flowValues = new Dictionary<string, string>();
                        flowValues.Add("phone", phone);
                        flowValues.Add("fullname", fullname);
                        flowValues.Add("ID", IDCode);
                        string metastring = JsonConvert.SerializeObject(flowValues);
                ...
                sendLink(fullname, phone, doctorfullname);
        ...
        private async Task<string> finalizePayment(paymentVM paymentModel, Guid userID, int flowID, int doctroFlowID)
        {
            ...
            using (Context dbcontext = new Context())
            {
                if (paymentModel.payment == "1")  // ینی پرداخت با کد تخفیف
                {
                ...
                else
                {
                    ...
                    flowRelation fln = new flowRelation()
                    {
                        ...
                        parentID = doctroFlowID,
                        formID = 7,
                        status = 0,
                        childStartDate = DateTime.Now,
                        childEndDate = DateTime.Now.AddMonths(paymentModel.month)
                    };
                    dbcontext.flowRelations.Add(fln);
                    await dbcontext.SaveChangesAsync();
                    paymentRecord selectedPeyment = await dbcontext.paymentRecords.SingleOrDefaultAsync(x => x.paymentRecordID == paymentModel.paymentRecortID);
                    selectedPeyment.relationID = fln.flowRelationID;
                    await dbcontext.SaveChangesAsync();
                }
        ...
        private async Task<paymentVM> paymentProcess(getURLVM model, Guid userID, string flowID)
        {
            using (Context dbcontext = new Context())
            {
                ...
                Guid orderOptionID = new Guid(plan.value.Split(':').ToList()[0]);
                orderOption planOption = await dbcontext.orderOptions.SingleOrDefaultAsync(x => x.orderOptionID == orderOptionID);
                price = double.Parse(planOption.Value.Split(',')[0]);
                month = Int32.Parse(planOption.Value.Split(',')[1]);

                var discount = lstform.SingleOrDefault(x => x.key == "discount");
                string payment = "";
                string message = "";
                paymentVM response = new paymentVM();
                // فعلا این حالتو نداریم ینی فقط پرداخت داریم
                if (false) // discount != null ینی کاربر تلاش کرده با کدتخفیف پرداخت کنه
                {
                    // یا موفق
                    payment = "1";
                    // یا غیر موفق
                ...
                {
                    // اینجا حتما میره بانک
                    payment = "2";
                    Random rnd = new Random();
                    int peigiry = rnd.Next(1000000, 9999999);
                    // اینجا باید ایتم مالی تولید شه و با پیگیری ارسال بشه به کنترل جدید
                    Guid peyguid = Guid.NewGuid();
                    paymentRecord paymentr = new paymentRecord()
                    {
                        status = 0,//پرداخت نشده  // 
                        paymentRecordID = peyguid,
                        peigiry = peigiry,
                        userID = new Guid(model.firstID),
                        planID = plan.value,
                        moneyFormID = plan.formID,
                        timestamp = dateTimeConvert.ConvertDateTimeToTimestamp(DateTime.Now),
                        price = price,
                    };
                    dbcontext.paymentRecords.Add(paymentr);
                    response.month = month;
                    response.paymentRecortID = peyguid;

                    response.payment = payment; // ینی پرداخت با کد تخفیف
                    response.status = 200;
                    await dbcontext.SaveChangesAsync();
        ...
        private async Task<string> changeFlowStatus(getURLVM model, Guid userID)
        {
            ...
            using (Context dbcontext = new Context())
            {
                ...
                if (status == 2 || status == 8)
                {
                    ...
                    flowLog newflowlog = new flowLog()
                    {
                        actionFlowID = flow.newOrderFlowID,
                        baseFlowID = flow.newOrderFlowID,
                        actorFlowID = userFlow.newOrderFlowID,
                        actorUserType = selectedUser.userType,
                        formID = (int)flow.formID,
                        userID = userFlow.userID,
                        creationDate = DateTime.Now,
                        actionTitle = "تایید توسط پزشک",
        ...
        private async Task<string> changeRelationStatus(getURLVM model, Guid userID)
        {
            string response = "";
            try
            {
                ...
                using (Context dbcontext = new Context())
                {
                    ...
                    // انیجا فلو کاربر باید پیدا بشه که به اردر کاربر فلو اضافه بشه
                    newOrderFlow userFlow = await dbcontext.newOrderFlows.SingleOrDefaultAsync(x => x.userID == userID && x.formID == baseFormID);//اولین فلو ثبت اطلاعات کاربر
                    flowRelation flow = await dbcontext.flowRelations.SingleOrDefaultAsync(x => x.childID == userFlow.newOrderFlowID && x.parentID == flowID); //
                    flow.status = 4;
        ...
        private async Task<string> doSaveForm(List<detailCollection> collection, int flowID, Context dbcontext)
        {
            ...
            await dbcontext.SaveChangesAsync();
            result = JsonConvert.SerializeObject(flowValues);
            return result;
        }
        private async Task<string> doEditForm(List<detailCollection> lstform, newOrderFlow flow, Context dbcontext)
        {
            Dictionary<string, string> flowValues = new Dictionary<string, string>();
            if (!string.IsNullOrEmpty(flow.meta))
            {
                flowValues = JsonConvert.DeserializeObject<Dictionary<string, string>>(flow.meta);
            }
            string result = "";
            ...
            foreach (var item in lstform)
            {
                if (!string.IsNullOrEmpty(item.value))
                {
                    List<detailCollection> detailList = lstform.Where(x => x.key == item.key).ToList();
                    string finalVAL = "";
                    ...
                    if (!flowValues.ContainsKey(item.key))
                    {
                        flowValues.Add(item.key, finalVAL);
                    }
                    else
                    {
                        flowValues[item.key] = finalVAL;
                    }
                    newOrderFields neworderfiled = await dbcontext.newOrderFields.FirstOrDefaultAsync(x => x.newOrderFlowID == flow.newOrderFlowID && x.formItemID == item.formItemID);
                    string fieldToGo = "";
                    switch (item.formItemTypeCode)
                    {
                        case ("6"): // انتخابی
                    ...
                    if (neworderfiled == null)
                    {
                        ...
                        neworderfiled.newOrderFlowID = flow.newOrderFlowID;
            ...
            await dbcontext.SaveChangesAsync();
            List<newOrderFields> lastItem = await dbcontext.newOrderFields.Where(x => x.newOrderFlowID == flow.newOrderFlowID).ToListAsync();
        ...
        private async Task<string> handleRegistration(getURLVM model, string phoneInserted)
        {
            ...
            try
            {
                ...
                if (model.data.ContainsKey("phone"))
                {
                ...
                using (Context dbcontext = new Context())
                {
                    ...
                    if (myuser != null)
                    {
                        ...
                        if (dynamicMethods.IsValidPhone(myuser.phone))
                        {
                            ...
                            dbcontext.SaveChanges();
                            responseModel smsResult = sendSMS(myuser.phone, num.ToString());
                            if (smsResult.status == 400)
                            {
                                return smsResult.message;
                            }
                        }
        ...
        private async Task<initDataVM> setTailorForm(getURLVM model, Guid userID)
        {
            ...
            try
            {
                using (Context dbcontext = new Context())
                {
                    ...
                    newOrderFlow selectedflow = new newOrderFlow();
                    if (finaldic.ContainsKey("flowID"))
                    {
                         selectedflow = await dbcontext.newOrderFlows.FirstOrDefaultAsync(x => x.newOrderFlowID == flowID);
                    ...
                    else
                    {
                        ...
                        dbcontext.newOrderFlows.Add(selectedflow);
                    }
                    ...
                    foreach (var item in lstform)
                    {
                        ...
                        fieldItem.name = item.key;
                        fieldItem.newOrderFieldsID = Guid.NewGuid();
                        fieldItem.newOrderFlowID = selectedflow.newOrderFlowID;
                        fieldItem.usedFeild = fieldToGo;
                        fieldItem.valueInt = 0;
                        fieldItem.valueDuoble = 0;
                        fieldItem.valueDateTime = DateTime.Now;
        ...
        private async Task<string> editProfileHandler(getURLVM model, Guid userID)
        {
            ...
            using (Context dbcontext = new Context())
            {
                newOrderFlow userFlow = await dbcontext.newOrderFlows.SingleOrDefaultAsync(x => x.userID == userID && x.formID == 983073566);//اولین فلو ثبت اطلاعات کاربر
                user user = await dbcontext.users.SingleOrDefaultAsync(x => x.userID == userID);
                List<detailCollection> lstform = JsonConvert.DeserializeObject<List<detailCollection>>(model.form);
                string name = lstform.SingleOrDefault(x => x.key == "Fullname").value;
                if (userFlow == null)
                {
                    userFlow = new newOrderFlow()
                    {
                        creationDate = DateTime.Now,
                        isFinished = "0",
                        isAccepted = "0",
                    ...
                    dbcontext.newOrderFlows.Add(userFlow);
                    await dbcontext.SaveChangesAsync();
                }

                userFlow.meta = await doSaveForm(lstform, userFlow.newOrderFlowID, dbcontext);
                await dbcontext.SaveChangesAsync();
                    ...
                    if (process == null)
                    {
                        ...
                        if (string.IsNullOrEmpty(fil.formImage))
                        {
                            ...
                            if (multiSelectITems.Count() > 0)
                            {
                                ...
                                List<orderOptionVM> orderotpinVMForSelectedOptions = await dbcontext.orderOptions.Where(x => orderOptionID.Contains(x.orderOptionID)).Select(g => new orderOptionVM { image = g.image, orderOptionID = g.orderOptionID, parentID = g.parentID, title = g.title }).ToListAsync();
        ...
        private async Task<int> getUserActionCount(string userID)
        {
            int flowListCount = 0;
            using (Context dbcontext = new Context())
            {
                Guid userGuid = new Guid(userID);
                newOrderFlow firstFlow = await dbcontext.newOrderFlows.Include(x => x.childFlows).SingleOrDefaultAsync(x => x.formID == 5 && x.userID == userGuid);
                var flowListQuery = firstFlow.childFlows.Where(x => x.formID == 31 && (x.status == 1 || x.status == 2)).ToList();
                //دریافت لیست فلو ها
                flowListCount = flowListQuery.Select(x => x.parentID).ToList().Count();
            }

            return flowListCount;


        }
        private async Task<int> getUserRecommendationCount(string userID)
        {
            int flowListCount = 0;
            using (Context dbcontext = new Context())
            {
                Guid userGuid = new Guid(userID);
                newOrderFlow firstFlow = await dbcontext.newOrderFlows.Include(x => x.childFlows).SingleOrDefaultAsync(x => x.formID == 5 && x.userID == userGuid);
                var flowListQuery = firstFlow.childFlows.Where(x => (x.formID == 61 || x.formID == 25) && (x.status == 1 || x.status == 2)).ToList();
                List<flowRelation> lst = flowListQuery.ToList();
                //دریافت لیست فلو ها
                flowListCount = flowListQuery.Select(x => x.parentID).ToList().Count();
            }
        ...
        public async Task<JObject> getTab([FromBody] getTabVM model)
        {
            ...
            {
                if (item.startPage == "app/clientActionList")
                {
                    int count = await getUserActionCount(someObject.ToString());

                    item.badge = count;
                }
                else if (item.startPage == "app/clientRecommendationList")
                {
                    int count = await getUserRecommendationCount(someObject.ToString());
                    item.badge = count;
                }
            }
            ...
            if (model.data != null)
            {
                if (splashModel.tabs != null)
                {
                    foreach (var item in splashModel.tabs)
                    {
                        int index = splashModel.tabs.IndexOf(item) + 1;
        ...
        public async Task<categoryPageVM> getCategoryList([FromBody] doSignIn model)
        {
            ...
            using (Context dbcontext = new Context())
            {
                response.list = await dbcontext.categories.Include(x => x.sectionType).Select(x => new categoryVM { sectionTypeName = x.sectionType.title, title = x.title, categoryID = x.categoryID }).ToListAsync();
                response.typelist = await dbcontext.sectionTypes.Select(x => new typeVM { title = x.title, typeID = x.sectionTypeID }).ToListAsync();
            }
            return response;
        }
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (exist == null)
                        {
                            ...
                            response.status = 200;
                            response.message = newSectionID.ToString();
                        }
                        else
                        {
                            response.status = 400;
        ...
        //forweb
        [System.Web.Http.HttpPost]
        public async Task<contentListVM> getSubHTML([FromBody] sectionVM model)
        {
            contentListVM output = new contentListVM();
            using (Context dbcontext = new Context())
            {
                try
                {
                    ...
                    var htmlquery = dbcontext.htmls.Where(x => x.parentID == parentHTML).AsQueryable();
                    output.htmlList = await htmlquery.Select(x => new HTMLVM { partialView = x.partialView, image = x.image, htmlName = x.title, htmlID = x.htmlID }).ToListAsync();
        ...
        private responseModel sendLink(string userName, string usePhone, string doctorName)
        {
            responseModel output = new responseModel();
            ...
            try
            {
                using (WebClient client = new WebClient())
                {
                    ...
                    collection.Add("receptor", usePhone);
                    collection.Add("sender", "200050001121");
                    collection.Add("token", ".");
                    collection.Add("token10", userName);
                    collection.Add("token20", doctorName);
                    collection.Add("template", "downloadLink");
        ...
        private responseModel sendSMS(string phone, string code)
        {
            responseModel output = new responseModel();
            string url = "https://api.kavenegar.com/v1/4D44716B654734752F4F774B5234554A724257426A756550475A36554F4C31445374574935434E647A79343D/verify/lookup.json";
            ...
            try
            {
                ...
                using (Context dbcontext = new Context())
                {
                    ...
                    if (model.formItemID != 0)
                    {
                        ...
                        formItem.isHidden = model.hiddenCheckBox == "on" ? 1 : 0;
            ...
            try
            {
                ...
                using (Context dbcontext = new Context())
                {
                    ...
                    if (model.orderOptionID != new Guid("00000000-0000-0000-0000-000000000000"))
                    {
                        ...
                        if (!string.IsNullOrEmpty(model.image))
                        {
                            mymodel.message = !string.IsNullOrEmpty(selecteOption.image) ? selecteOption.image : "";
                            selecteOption.image = model.image;
                        }
                        selecteOption.parentID = model.parentID;
                        selecteOption.title = model.title;
                        selecteOption.Value = model.value;
                        selecteOption.priority = model.priority;
                        mymodel.status = 200;
                        await dbcontext.SaveChangesAsync();
                    ...
                    {
                        var orop = await dbcontext.orderOptions.FirstOrDefaultAsync(i => i.title == model.title && i.parentID == model.parentID);
                        if (orop == null)
                        {
                            Guid ptid = Guid.NewGuid();
                            Guid parenttid = ptid;
        ...
        [BasicAuthentication]
        [System.Web.Http.HttpPost]
        public async Task<JObject> removeOrderOption([FromBody] addOrderOptionVM model)
        {
            responseModel mymodel = new responseModel();
            try
            {
                ...
                using (Context dbcontext = new Context())
                {
                    var prolist = await dbcontext.orderOptions.ToListAsync();
                    var product = await dbcontext.orderOptions.FirstOrDefaultAsync(x => x.orderOptionID == model.orderOptionID);
                    if (product != null)
                    {
                        dbcontext.orderOptions.Remove(product);
